...
export const findNearestIdxOnOrBefore = <T>(target: DateTime, data: T[], selector: (data: T) => DateTime) => {
  return data.findLastIndex((x) => selector(x).startOf("day") <= target.startOf("day"));
};
